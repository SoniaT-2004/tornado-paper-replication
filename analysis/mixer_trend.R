...
# Summarize daily total transaction count (ETH + ERC20 if desired)
df_daily <- df %>%
...
# Define sanction date and add time variables
...
# smooth series (30-day moving average)
df_daily <- df_daily %>%
  mutate(daily_tx_ma7 = zoo::rollmean(daily_tx, 7, fill = NA, align = "right"))

# model with delayed effect
# use a lag window (e.g., 7 days) to allow gradual change
df_daily <- df_daily %>%
