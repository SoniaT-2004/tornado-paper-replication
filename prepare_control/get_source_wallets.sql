...
-- 1. All inflows (ETH + any ERC-20, converted to USD equivalent)
inflows AS (
  -- ETH part
  SELECT
  ...
  JOIN ethereum.transactions t
   ...
   AND t.block_time > TIMESTAMP '2019-01-01'
  
  UNION ALL
  
  -- ERC-20 part (any token, >= $10 equivalent)
  SELECT
    w.wallet AS target_wallet,
  ...
  JOIN erc20_ethereum.evt_Transfer e
   ...
   AND e.evt_block_time > TIMESTAMP '2019-01-01'
...
-- 2. Completely original aggregation
src_summary AS (
  SELECT
    target_wallet,
    source_wallet,
...
-- 3. Source wallet historical activity
src_activity AS (
  SELECT
    "from"                                              AS wallet,
